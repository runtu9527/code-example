...
namespace SerialPortHelper
{
    ...
    public class SerialPortUtil
    {
        ...
        public byte EndByte = 0x3E;//string End = ">";
        //public byte EndByte = 161;//string End = ">";
        /// <summary>
        /// 完整协议的记录处理事件
        /// </summary>
        ...
        public SerialPortUtil(string name, SerialPortBaudRates baud, Parity par, SerialPortDatabits dBits, StopBits sBits)
        {
            ...
            //SetPortParas(_portName, _baudRate, _parity, _dataBits, _stopBits);
        ...
        {
#if TEST
            OutputDebugString("begin comPort_DataReceived: ");
#endif
            //禁止接收事件时直接退出
...
#if TEST
                        string str = HexConverter.ByteToHex(_byteData.ToArray());
            ...
            #endregion
#if TEST
                //OutputDebugString("SerialPortUtil.comPort_DataReceived before readString: ");
#endif
                //string readString1 = HexConverter.ByteToHex(_byteData.ToArray());
                string readString = HexConverter.ByteToString(_byteData.ToArray());
#if TEST
                //OutputDebugString("SerialPortUtil.comPort_DataReceived after readString: " + readString);
                //OutputDebugString("DMSDebug:BytetoString is: " + readString);
                //OutputDebugString("DMSDebug:ReadData Count is " + readString.Length.ToString());
#endif
                //字符转换
                //string readString = System.Text.Encoding.Default.GetString(_byteData.ToArray(), 0, _byteData.Count);

                //OutputDebugString("SerialPortUtil.comPort_DataReceived return readString:  " + readString);
                //触发整条记录的处理HexConverter.ByteToString(_byteData.ToArray));
        ...
        {
#if TEST
            OutputDebugString("comPort_ErrorReceived ");
#endif
            if (Error != null)
            {
#if TEST
        ...
        {
#if TEST
            OutputDebugString("WriteData:" + msg);
#endif
        ...
        {
#if TEST
            OutputDebugString("DMSDebug WriteData:" + HexConverter.ByteToString(msg));
#endif
            if (!(comPort.IsOpen)) comPort.Open();
        ...
        {
#if TEST
            //OutputDebugString("WriteData:" + HexConverter.ByteToHex(msg));
#endif
            if (!(comPort.IsOpen)) comPort.Open();
            try
            {
        ...
        public int SendCommand(byte[] SendData, ref  byte[] ReceiveData, int Overtime, byte ReceiveLength)
        {
            if (!(comPort.IsOpen)) comPort.Open();
            ...
            if (comPort.BytesToRead >= ReceiveLength)
            {
                ret = comPort.Read(ReceiveData, 0, ReceiveData.Length);
            }
